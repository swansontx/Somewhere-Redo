...
final class DropStore: ObservableObject {
    ...
    init() {
        authListener = Auth.auth().addStateDidChangeListener { [weak self] _, user in
            // Use optional chaining to avoid strengthening `self` inside an escaping closure
...
deinit {
        ...
        // deinit can run on a non-isolated context; ensure we call actor-isolated API safely
        Task { @MainActor in
            self.stopListeningNearby()
        }
    }
    ...
    func toggleLift(_ drop: DropItem) {
        ...
        if let i = drops.firstIndex(where: { $0.id == drop.id }) {
            ...
            if lifted.contains(drop.id) {
                ...
                drops[i].liftCount = max(0, drops[i].liftCount - 1)
            }
        ...
        if var remote = remoteDrops[drop.id] {
            if lifted.contains(drop.id) {
                ...
                remote.liftCount = max(0, remote.liftCount - 1)
            }
        ...
        if var optimistic = optimisticDrops[drop.id] {
            if lifted.contains(drop.id) {
                optimistic.liftCount += 1
            } else {
                optimistic.liftCount = max(0, optimistic.liftCount - 1)
            }
    ...
    func react(to drop: DropItem) {
        if let i = drops.firstIndex(where: { $0.id == drop.id }) {
            drops[i].reactionCount += 1
            drops[i].hasReacted = true
        }
        if var remote = remoteDrops[drop.id] {
            remote.reactionCount += 1
            remote.hasReacted = true
            remoteDrops[drop.id] = remote
        }
        if var optimistic = optimisticDrops[drop.id] {
            optimistic.reactionCount += 1
            optimistic.hasReacted = true
            optimisticDrops[drop.id] = optimistic
        }
        publishDrops()
    }
    ...
    /// Returns whether the given drop is currently being updated/synced
    func isUpdating(dropId: String) -> Bool {
        if let optimistic = optimisticDrops[dropId] {
            if case .pending = optimistic.syncStatus { return true }
        }
        return false
    }
